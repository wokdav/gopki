...
func EncryptPKCS1v15(random io.Reader, pub *internalRsa.PublicKey, msg []byte) ([]byte, error) {
	k := pub.Size()
	if len(msg) > k-11 {
		return nil, internalRsa.ErrMessageTooLong
	}
...
func DecryptPKCS1v15SessionKey(random io.Reader, priv *internalRsa.PrivateKey, ciphertext []byte, key []byte) error {
	...
	k := priv.Size()
	if k-(len(key)+3+8) < 0 {
		return internalRsa.ErrDecryption
	}
...
func decryptPKCS1v15(priv *internalRsa.PrivateKey, ciphertext []byte) (valid int, em []byte, index int, err error) {
	k := priv.Size()
	if k < 11 {
		err = internalRsa.ErrDecryption
		return
	}
...
func VerifyPKCS1v15(pub *internalRsa.PublicKey, hash crypto.Hash, hashed []byte, sig []byte) error {
	...
	k := pub.Size()
	if k < tLen+11 {
		return internalRsa.ErrVerification
	}
